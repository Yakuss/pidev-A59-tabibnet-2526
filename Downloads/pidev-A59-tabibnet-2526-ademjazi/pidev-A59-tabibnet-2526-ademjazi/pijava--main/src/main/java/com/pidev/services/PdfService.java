...
public class PdfService {
    ...
    public void generateMedicalDossier(Document doc, List<Rapport> rapports, List<Ordonnance> ordonnances,
        ...
        addInfoRow(infoGrid, "Nom du fichier", doc.getNomFichier(), boldFont, normalFont, GRAY_100, GRAY_200);
        addInfoRow(infoGrid, "Type de dossier", doc.getType(), boldFont, normalFont, GRAY_100, GRAY_200);
        addInfoRow(infoGrid, "Date de création", doc.getDateCreation() != null ? doc.getDateCreation().toString() : "N/A", boldFont, normalFont, GRAY_100, GRAY_200);
        if (doc.getDescription() != null) {
            addInfoRow(infoGrid, "Description", doc.getDescription(), boldFont, normalFont, GRAY_100, GRAY_200);
        }
        pdfDoc.add(infoGrid);
    ...
    private void addInfoRow(PdfPTable table, String label, String value, Font labelFont, Font valueFont, Color labelBg, Color borderColor) {
        PdfPCell l = new PdfPCell(new Phrase(label, labelFont));
        l.setBackgroundColor(labelBg);
        l.setPadding(8);
    ...
    private static class RoundedCellEvent implements PdfPCellEvent {
        private int radius;
        public RoundedCellEvent(int r) { this.radius = r; }
        @Override
    ...
    private static class MedicalPageEvent extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, com.lowagie.text.Document document) {
            ...
            cb.beginText();
            try { cb.setFontAndSize(BaseFont.createFont(), 9); } catch (Exception e) {}
